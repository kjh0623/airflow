...
class TestSchedulerJob:
    ...
    def test_queued_dagrun_with_unresolvable_pinned_version_is_stuck_forever(self, dag_maker, session):
        """
        A queued run whose pinned dag version can no longer be resolved is skipped
        silently on every scheduler loop: it is neither started nor failed.
        ...
        remains set, so DBDagBag._version_from_dag_run() returns None and never
        falls back to the latest version.
        """
        ...
        # The scheduler skips the run on every loop: still QUEUED, never failed,
        # and no timeout ever applies.
        assert dr.state == State.QUEUED
